...
func CreateTestConfig(t *testing.T, name string, mode Mode, opts ...ConfigOption) string {
	...
	if options.monitoring {
		// E2E tests use ephemeral storage (no persistence) to avoid PVC binding timeouts
		content.WriteString("\n# Monitoring with ephemeral storage (no persistence)\n")
		content.WriteString("kube_prometheus_stack:\n")
		content.WriteString("  enabled: true\n")
		content.WriteString("  prometheus:\n")
		content.WriteString("    persistence:\n")
		content.WriteString("      enabled: false\n")
		content.WriteString("  grafana:\n")
		content.WriteString("    persistence:\n")
		content.WriteString("      enabled: false\n")
	}
...
func SimulateNodeFailure(ctx context.Context, t *testing.T, state *OperatorTestContext, nodeName string) error {
	...
	for _, server := range servers {
		if server.Name == nodeName {
			...
			t.Logf("Node %s powered off (server ID: %d)", nodeName, server.ID)

			// Verify the server is actually off
			verifyCmd := exec.CommandContext(ctx, "hcloud", "server", "describe", fmt.Sprintf("%d", server.ID), "-o", "format={{.Status}}")
			verifyOutput, _ := verifyCmd.CombinedOutput()
			t.Logf("Server %s status after poweroff: %s", nodeName, strings.TrimSpace(string(verifyOutput)))
...
func WaitForNodeNotReadyK8s(ctx context.Context, t *testing.T, kubeconfigPath, nodeName string, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		...
		if elapsed > 60*time.Second && int(elapsed.Seconds())%30 == 0 {
			t.Logf("Node %s still Ready after %s, waiting...", nodeName, elapsed.Round(time.Second))
		}

		time.Sleep(10 * time.Second)
	}

	return fmt.Errorf("timeout waiting for node %s to become NotReady after %s", nodeName, timeout)
}
