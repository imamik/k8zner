...
func TestApplyLoadBalancerRDNS_IngressRole(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := &config.Config{
		ClusterName: "test-cluster",
	...
	var capturedDNS string
	mockInfra.SetLoadBalancerRDNSFunc = func(_ context.Context, _ int64, _, dnsPtr string) error {
		capturedDNS = dnsPtr
		return nil
	}
	...
	err := p.applyLoadBalancerRDNS(ctx, 2, "test-cluster-ingress", "5.6.7.8", "", "ingress")
	require.NoError(t, err)
	assert.Equal(t, "test-cluster-ingress.ingress.example.com", capturedDNS)
}

func TestApplyLoadBalancerRDNS_IngressFallsBackToClusterRDNS(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := &config.Config{
		ClusterName: "test-cluster",
		RDNS: config.RDNSConfig{
			ClusterRDNS: "{{ hostname }}.cluster.example.com",
		},
	}
	...
	err := p.applyLoadBalancerRDNS(ctx, 2, "test-cluster-ingress", "5.6.7.8", "", "ingress")
	require.NoError(t, err)
	assert.Equal(t, "test-cluster-ingress.cluster.example.com", capturedDNS)
}
...
// --- Ingress LB error paths ---

func TestProvisionLoadBalancers_IngressEnsureError(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := newTestConfig(t)
	...
	mockInfra.EnsureLoadBalancerFunc = func(_ context.Context, name, _, _ string, _ hcloud.LoadBalancerAlgorithmType, _ map[string]string) (*hcloud.LoadBalancer, error) {
		lbCallCount++
		if lbCallCount == 2 {
			// Fail on ingress LB (second call)
			return nil, fmt.Errorf("ingress LB creation failed")
		}
		return &hcloud.LoadBalancer{
			ID:   1,
			Name: name,
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ingress LB creation failed")
}

func TestProvisionLoadBalancers_IngressServiceError(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := newTestConfig(t)
	cfg.Ingress = config.IngressConfig{Enabled: true}
	...
	mockInfra.ConfigureServiceFunc = func(_ context.Context, _ *hcloud.LoadBalancer, svc hcloud.LoadBalancerAddServiceOpts) error {
		serviceCallCount++
		// API LB has 2 services (6443, 50000), ingress port 80 is the 3rd call
		if serviceCallCount == 3 {
			return fmt.Errorf("ingress service 80 failed")
		}
		return nil
	}
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ingress service 80 failed")
}

func TestProvisionLoadBalancers_IngressAttachNetworkError(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := newTestConfig(t)
	cfg.Ingress = config.IngressConfig{Enabled: true}
	...
	mockInfra.AttachToNetworkFunc = func(_ context.Context, _ *hcloud.LoadBalancer, _ *hcloud.Network, _ net.IP) error {
		...
		if attachCallCount == 2 {
			// Fail on ingress LB network attach (second call)
			return fmt.Errorf("ingress network attach failed")
		}
		return nil
	}
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ingress network attach failed")
}

func TestProvisionLoadBalancers_IngressAddTargetError(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := newTestConfig(t)
	cfg.Ingress = config.IngressConfig{Enabled: true}
	...
	addTargetCallCount := 0
	mockInfra.AddTargetFunc = func(_ context.Context, _ *hcloud.LoadBalancer, _ hcloud.LoadBalancerTargetType, _ string) error {
		addTargetCallCount++
		if addTargetCallCount == 2 {
			// Fail on ingress LB target add (second call)
			return fmt.Errorf("ingress target add failed")
		}
		return nil
	}
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to add target to Ingress LB")
}

func TestProvisionLoadBalancers_IngressWithRDNS(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := newTestConfig(t)
	...
	var rdnsCalls []string
	mockInfra.SetLoadBalancerRDNSFunc = func(_ context.Context, _ int64, _, dnsPtr string) error {
		rdnsCalls = append(rdnsCalls, dnsPtr)
		return nil
	}
	...
	require.NoError(t, err)
	// Should have RDNS call for ingress LB
	require.NotEmpty(t, rdnsCalls)
}
