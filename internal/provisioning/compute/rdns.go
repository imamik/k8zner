...
func (p *Provisioner) applyLoadBalancerRDNS(ctx *provisioning.Context, lbID int64, lbName, ipv4, ipv6, role string) error {
	...
	if rdnsIPv4 != "" && ipv4 != "" {
		...
		// Set RDNS with retry logic
		if err := retryRDNS(ctx, func() error {
			return ctx.Infra.SetLoadBalancerRDNS(ctx, lbID, ipv4, dnsPtr)
		}, fmt.Sprintf("load balancer %d (%s)", lbID, lbName)); err != nil {
			return fmt.Errorf("failed to set IPv4 RDNS for %s (IP: %s → %s): %w", lbName, ipv4, dnsPtr, err)
		}
	...
	if rdnsIPv6 != "" && ipv6 != "" {
		...
		// Set RDNS with retry logic
		if err := retryRDNS(ctx, func() error {
			return ctx.Infra.SetLoadBalancerRDNS(ctx, lbID, ipv6, dnsPtr)
		}, fmt.Sprintf("load balancer %d (%s)", lbID, lbName)); err != nil {
			return fmt.Errorf("failed to set IPv6 RDNS for %s (IP: %s → %s): %w", lbName, ipv6, dnsPtr, err)
		}
