...
func TestEnsureServer_RDNS(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := testConfigWithSubnets(t)
	tracker := newServerTracker()
	...
	mockInfra.GetServerIDFunc = tracker.getID
	mockInfra.CreateServerFunc = tracker.create("42")
	mockInfra.GetServerIPFunc = func(_ context.Context, _ string) (string, error) {
		return "203.0.113.10", nil
	}
	mockInfra.GetSnapshotByLabelsFunc = func(_ context.Context, _ map[string]string) (*hcloud.Image, error) {
	...
	rdnsCalled := false
	mockInfra.SetServerRDNSFunc = func(_ context.Context, serverID int64, ipAddress, dnsPtr string) error {
		rdnsCalled = true
		assert.Equal(t, int64(42), serverID)
		assert.Equal(t, "203.0.113.10", ipAddress)
		assert.NotEmpty(t, dnsPtr)
		return nil
	}
...
func TestEnsureServer_RDNSErrorIsNonFatal(t *testing.T) {
	t.Parallel()
	mockInfra := &hcloud_internal.MockClient{}
	cfg := testConfigWithSubnets(t)
	tracker := newServerTracker()
	...
	mockInfra.SetServerRDNSFunc = func(_ context.Context, _ int64, _, _ string) error {
		return fmt.Errorf("RDNS service unavailable")
	}

	ctx := createTestContext(t, mockInfra, cfg)
	p := NewProvisioner()

	// RDNS failure should not fail server creation
	info, err := p.ensureServer(ctx, ServerSpec{
		Name:     "test-cluster-cp-1",
		Type:     "cx21",
		Location: "nbg1",
