...
func (m *MockClient) CreateServer(ctx context.Context, name, imageType, serverType string, sshKeys []string) (string, error) {
	if m.CreateServerFunc != nil {
		return m.CreateServerFunc(ctx, name, imageType, serverType, sshKeys)
	}
	return "mock-id", nil
}
