...
func (r *ClusterReconciler) scaleUpControlPlanes(ctx context.Context, cluster *k8znerv1alpha1.K8znerCluster, count int) error {
	...
	if talosClient == nil && cluster.Spec.CredentialsRef.Name != "" {
		...
		if err != nil {
			...
			// Discover LB info from HCloud if not already in status
			if cluster.Status.Infrastructure.LoadBalancerID == 0 || cluster.Status.Infrastructure.LoadBalancerIP == "" {
				...
				if lbErr == nil && lb != nil {
					...
					logger.Info("discovered LB info for CP scaling",
						"lbID", lb.ID,
	...
	logger.Info("creating ephemeral SSH key for CP scaling", "keyName", sshKeyName)
	...
	for i := 0; i < count; i++ {
		...
		if err != nil {
			...
			logger.Error(err, "failed to create CP server", "name", newServerName)
			r.Recorder.Eventf(cluster, corev1.EventTypeWarning, EventReasonServerCreationError,
		...
		logger.Info("created CP server", "name", newServerName)
		...
		if err != nil {
			...
			r.Recorder.Eventf(cluster, corev1.EventTypeWarning, EventReasonServerCreationError,
				"Failed to get IP for CP server %s: %v", newServerName, err)
			r.updateNodePhase(ctx, cluster, "control-plane", NodeStatusUpdate{
				Name:   newServerName,
				Phase:  k8znerv1alpha1.NodePhaseFailed,
		...
		// Use private IP for Talos communication if available
		talosIP := serverIP
		if privateIP != "" {
			talosIP = privateIP
		...
		// Step 7: Generate and apply Talos config
		if talosConfigGen != nil && talosClient != nil {
			...
			logger.Info("applying Talos config to CP", "name", newServerName, "ip", talosIP)
			...
			// Step 8: Wait for node to become ready
			r.updateNodePhase(ctx, cluster, "control-plane", NodeStatusUpdate{
				Name:   newServerName,
			...
			logger.Info("waiting for CP node to become ready", "name", newServerName, "ip", talosIP)
			r.updateNodePhase(ctx, cluster, "control-plane", NodeStatusUpdate{
				Name:   newServerName,
